 ...
 * @returns Array of active option IDs
 ...
 * );
 * console.log(result.data); // ['size-option', 'topping-option']
 * ```
 */
export const getActiveOptions = async (
  ...
  try {
    ...
    // Get active options using instance method
    const activeOptions = menu.getActiveOptions(itemId);

    return {
      success: true,
      data: activeOptions,
      count: activeOptions.length,
      message: `Found ${activeOptions.length} active option(s)`,
    };
  } catch (error: any) {
