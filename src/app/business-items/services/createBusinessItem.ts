 ...
 */
type CreateBusinessItemOptions = Pick<CreateOptions<any>, 'transaction'>;
 ...
 * @example
 * ```typescript
 * const newItem = await createBusinessItem({
...
export const createBusinessItem = async (
  ...
  options?: CreateBusinessItemOptions
  ...
  try {
    ...
    const existingItem = await BusinessItemModel.findOne({
      where: {
    ...
    if (data.sku) {
      const existingSku = await BusinessItemModel.findOne({
    ...
    if (data.barcode) {
      const existingBarcode = await BusinessItemModel.findOne({
    ...
    const normalizedData: Partial<BusinessItemAttributes> = {
      businessId: data.businessId,
      name: normalizeName(data.name),
      price: data.price,
    ...
    if (data.shortDescription) normalizedData.shortDescription = data.shortDescription.trim();
    if (data.category) normalizedData.category = data.category;
    if (data.discountPrice !== undefined) normalizedData.discountPrice = data.discountPrice;
    if (data.discountStartDate) normalizedData.discountStartDate = data.discountStartDate;
    if (data.discountEndDate) normalizedData.discountEndDate = data.discountEndDate;
    if (data.allergenNotes) normalizedData.allergenNotes = data.allergenNotes.trim();
    ...
    if (data.carbohydrates !== undefined) normalizedData.carbohydrates = data.carbohydrates;
    ...
    if (data.stockQuantity !== undefined) normalizedData.stockQuantity = data.stockQuantity;
    if (data.lowStockThreshold !== undefined) normalizedData.lowStockThreshold = data.lowStockThreshold;
    if (data.preparationTime !== undefined) normalizedData.preparationTime = data.preparationTime;
    if (data.maxOrderQuantity !== undefined) normalizedData.maxOrderQuantity = data.maxOrderQuantity;
    if (data.imageUrl) normalizedData.imageUrl = data.imageUrl.trim();
    if (data.thumbnailUrl) normalizedData.thumbnailUrl = data.thumbnailUrl.trim();
    ...
    const record = await BusinessItemModel.create(normalizedData as any, {
      transaction: options?.transaction,
    });
