 ...
 */
type UpdateBusinessItemOptions = Pick<UpdateOptions<any>, 'transaction'>;
 ...
 * @example
 * ```typescript
 * const updatedItem = await updateBusinessItem('item-uuid-here', {
 *   price: 14.99,
 *   available: false
 * });
 * ```
 */
export const updateBusinessItem = async (
  id: string,
  data: Partial<BusinessItemAttributes>,
  options?: UpdateBusinessItemOptions
  ...
  try {
    ...
    const newDiscountStartDate = data.discountStartDate ?? record.discountStartDate;
    ...
    const newMinOrderQuantity = data.minOrderQuantity ?? record.minOrderQuantity;
    const newMaxOrderQuantity = data.maxOrderQuantity ?? record.maxOrderQuantity;
    ...
    if (data.thumbnailUrl && shouldValidateField(record.thumbnailUrl, data.thumbnailUrl)) {
    ...
    if (data.carbohydrates !== undefined && data.carbohydrates !== record.carbohydrates) {
    ...
    if (data.stockQuantity !== undefined && data.stockQuantity !== record.stockQuantity) {
      validateQuantity(data.stockQuantity, 'Stock quantity', 0);
    }
    if (data.lowStockThreshold !== undefined && data.lowStockThreshold !== record.lowStockThreshold) {
      validateQuantity(data.lowStockThreshold, 'Low stock threshold', 0);
    }

    // Validate preparation time if provided
    if (data.preparationTime !== undefined && data.preparationTime !== record.preparationTime) {
    ...
    if (data.name && shouldValidateField(record.name, data.name)) {
      const existingName = await BusinessItemModel.findOne({
        where: {
    ...
    const updateData: Partial<BusinessItemAttributes> = { ...data };
