...
import { BusinessItemAttributes } from '../interfaces/business-item.interface.js';
import { BusinessItemModel, ItemType, SpicyLevel } from '../models/business-item.model.js';
 ...
 */
type BulkCreateBusinessItemOptions = {
  transaction?: BulkCreateOptions<any>['transaction'];
  validate?: boolean;
  ignoreDuplicates?: boolean;
  updateOnDuplicate?: (keyof BusinessItemAttributes)[];
};
...
const validateItemRecord = (
  data: Partial<BusinessItemAttributes>,
  index: number
): void => {
  ...
  if (!data.businessId) {
    throw new ValidationError('Validation failed', `${prefix} Business ID is required`);
  }

  if (!data.name) {
    throw new ValidationError('Validation failed', `${prefix} Item name is required`);
  }

  if (data.price === undefined || data.price === null) {
    throw new ValidationError('Validation failed', `${prefix} Price is required`);
  }
 ...
 * @example
 * ```typescript
 * const items = await bulkCreateBusinessItems([
 *   { businessId: 'uuid', name: 'Pizza', price: 12.99, type: 'food' },
 *   { businessId: 'uuid', name: 'Soda', price: 2.99, type: 'drink' },
 * ]);
 * ```
 */
export const bulkCreateBusinessItems = async (
  dataArray: Partial<BusinessItemAttributes>[],
  options?: BulkCreateBusinessItemOptions
  ...
  try {
    ...
    if (businessNamePairs.length !== uniqueBusinessNamePairs.size) {
      ...
      throw new ValidationError(
        'Validation failed',
        `Duplicate item names found within the same business: ${[...new Set(duplicates)].join(', ')}`
      );
    }
    ...
    if (skusInArray.length !== uniqueSkus.size) {
      ...
      throw new ValidationError(
        'Validation failed',
        `Duplicate SKUs found in the data array: ${[...new Set(duplicates)].join(', ')}`
      );
    }
    ...
    if (barcodesInArray.length !== uniqueBarcodes.size) {
      ...
      throw new ValidationError(
        'Validation failed',
        `Duplicate barcodes found in the data array: ${[...new Set(duplicates)].join(', ')}`
      );
    }
    ...
    if (existingBusinesses.length !== uniqueBusinessIds.length) {
      ...
      const missingIds = uniqueBusinessIds.filter((id) => !existingIds.includes(id));
      throw new ValidationError(
        'Invalid reference',
    ...
    if (inactiveBusinesses.length > 0) {
      throw new ValidationError(
        'Business inactive',
        `Cannot add items to inactive businesses: ${inactiveBusinesses.map((b) => b.businessId).join(', ')}`
      );
    }
    ...
    if (skusInArray.length > 0) {
      const existingSkus = await BusinessItemModel.findAll({
    ...
    if (barcodesInArray.length > 0) {
      const existingBarcodes = await BusinessItemModel.findAll({
    ...
    const normalizedData = dataArray.map((data) => {
      ...
      if (data.shortDescription) normalized.shortDescription = data.shortDescription.trim();
      if (data.category) normalized.category = data.category;
      if (data.discountPrice !== undefined) normalized.discountPrice = data.discountPrice;
      if (data.discountStartDate) normalized.discountStartDate = data.discountStartDate;
      if (data.discountEndDate) normalized.discountEndDate = data.discountEndDate;
      if (data.allergenNotes) normalized.allergenNotes = data.allergenNotes.trim();
      ...
      if (data.carbohydrates !== undefined) normalized.carbohydrates = data.carbohydrates;
      ...
      if (data.stockQuantity !== undefined) normalized.stockQuantity = data.stockQuantity;
      if (data.lowStockThreshold !== undefined) normalized.lowStockThreshold = data.lowStockThreshold;
      if (data.preparationTime !== undefined) normalized.preparationTime = data.preparationTime;
      if (data.maxOrderQuantity !== undefined) normalized.maxOrderQuantity = data.maxOrderQuantity;
    ...
    const records = await BusinessItemModel.bulkCreate(normalizedData as any[], {
      transaction: options?.transaction,
    ...
    return {
      success: true,
      data: records,
      message: `Successfully created ${records.length} business item${records.length > 1 ? 's' : ''}`,
    };
  } catch (error) {
    // ========================================================================
    // ERROR HANDLING
