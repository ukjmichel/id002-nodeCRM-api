...
import { updateBusinessItem } from '../services/updateBusinessItem.js';
 ...
 *
 * @example
 * PUT /api/business-items/123e4567-e89b-12d3-a456-426614174000
 * Body: {
 *   "price": 14.99,
 *   "available": false,
 *   "isVegan": true
 * }
 */
export const updateBusinessItemController = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    const result = await withTransaction(async (transaction) => {
      return await updateBusinessItem(id, req.body, { transaction });
    });
